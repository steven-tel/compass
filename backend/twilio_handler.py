...
from db import HomeworkSessionRecorder
...
class TwilioHandler:
    ...
    async def handle_media_stream(self, websocket):
        ...
        gemini_task = asyncio.create_task(self._run_gemini_session(
            ...
            audio_output_callback, audio_interrupt_callback, recorder
        ))
        ...
        try:
            async for message in websocket.iter_text():
                ...
                if event == "start":
                    ...
                    await text_input_queue.put(
                        "The student just called. Greet them in one short sentence as their tutor. "
                        "Ask what exercise they are working on. Do not mention Gemini, demos, or product features."
                    )
                elif event == "media":
            ...
            try:
                ...
                if saved.raw_transcript_ref:
                    ...
                    from db import set_session_analysis_status
    ...
    async def _run_gemini_session(self, audio_input_queue, video_input_queue, text_input_queue, output_callback, interrupt_callback, recorder):
        try:
